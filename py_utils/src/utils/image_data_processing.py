...
def convert_subfolder_images(
    input_dir_path="epo/samples/full-text", output_dir_path="static/img/patent_images"
):
        ...
        for file in os.listdir(f"{input_dir_path}/{folder}"):
            if file.endswith(".tif"):
                if file.startswith("imgf"):
                    convert_tiff_to_png(
                        f"{input_dir_path}/{folder}/{file}",
                        f'{output_dir_path}/{file.replace(".tif", "")}{folder}.png',
                    )
